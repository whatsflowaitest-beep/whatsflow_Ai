...
async function benchmark(name, fn, runs, thresholdP95Ms) {
    const times = [];
...
// ── Test Suites ───────────────────────────────────────────────────────────────
async function testInboxQuery(tenantId) {
    return benchmark('Inbox query (conversations + contacts join, tenant-scoped)', async () => {
        await supabase
            .from('conversations')
            ...
            .limit(25);
    }, 50, 100 // p95 must be ≤100ms
    );
}
async function testMessagePagination(tenantId) {
    ...
    return benchmark('Message pagination (50 messages per page)', async () => {
        await supabase
            .from('messages')
            ...
            .limit(50);
    }, 50, 80 // p95 must be ≤80ms
    );
}
async function testRLSOverhead(tenantId, userToken) {
    // Compare service-role (no RLS) vs anon (RLS active) query time
    const serviceRoleTimes = [];
    const rlsTimes = [];
    const runs = 20;
    for (let i = 0; i < runs; i++) {
    ...
    const status = passed ? '✅' : '❌';
    console.log(`${status} [RLS overhead estimation]\n` +
...
async function testConcurrentWrites(tenantId) {
    ...
    for (let batch = 0; batch < 5; batch++) {
        const t0 = performance.now();
        await Promise.all(Array.from({ length: CONCURRENT }, (_, i) => supabase.from('messages').insert({
            tenant_id: tenantId,
    ...
    console.log(`${passed ? '✅' : '❌'} [Concurrent writes (${CONCURRENT} parallel)]\n` +
...
async function testAIAgentLookup(tenantId) {
    return benchmark('Active AI agent lookup (partial index)', async () => {
        await supabase
            .from('ai_agents')
            ...
            .limit(1)
            .maybeSingle();
    }, 100, 30 // Should be <30ms with partial index
    );
}
...
//# sourceMappingURL=db.load.js.map