...
async function testThroughput(): Promise<void> {
  ...
  console.log(
    `   ${passed ? '✅' : '❌'} Processed ${processed}/${TOTAL} jobs in ${totalMs}ms` +
...
async function testDeduplication(): Promise<void> {
  ...
  console.log(
    `   ${passed ? '✅' : '❌'} Enqueued ${ATTEMPTS}x, processed ${processCount}x` +
...
async function testBurst(): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    const check = setInterval(() => {
      if (processed >= BURST) { clearInterval(check); resolve() }
    }, 200)
    setTimeout(() => { clearInterval(check); resolve() }, 30_000)
  })
  ...
  console.log(
    `   ${passed ? '✅' : '❌'} Burst ${BURST} jobs: enqueued in ${enqueueMs}ms, ` +
...
async function testDLQRouting(): Promise<void> {
  ...
  console.log(
    `   ${passed ? '✅' : '❌'} Job failed ${failCount} times, DLQ received ${dlqCount} entry` +
...
async function testRedisConnectivity(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `   ${passed ? '✅' : '⚠️'} Redis PING in ${latencyMs.toFixed(1)}ms` +
